            ...
            with self._lock:
                ...
                for session_id, session_data in old_sessions.items():
                    ...
                    if session_id not in sessions:
                        ...
                        try:
                            old_updated = datetime.fromisoformat(session_data.get("last_updated", ""))
                            current_updated = datetime.fromisoformat(sessions[session_id].get("last_updated", ""))
...
def migrate_from_settings_json() -> None:
    ...
    if settings_file.exists():
        ...
        if migrated > 0:
            print(f"Migrated {migrated} sessions from settings.json to state file", file=sys.stderr)
