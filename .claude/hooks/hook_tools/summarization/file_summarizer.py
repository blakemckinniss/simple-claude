...
def load_constants() -> Dict[str, Any]:
    ...
    constants_path = paths.constants_file
    try:
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                # Long method
                func_lines = (
                    content[node.lineno - 1 : node.end_lineno].count("\n")
...
def summarize_python_file(filepath: str) -> Dict[str, Any]:
    ...
    try:
        with open(
            ...
            errors=CONSTANTS["file_encoding"]["error_handling"],
        ) as f:
            source = f.read()
    ...
    summary["function_calls"] = dict(
        list(function_calls.items())[:5]
    ...
    if summary["functions"] or summary["classes"]:
        summary["description"] = CONSTANTS["file_summary"][
            "classes_functions_template"
        ].format(
    ...
    try:
        with open(
            ...
            for line in f:
                line = line.strip()
                if line:
                    truncate_len = CONSTANTS["text_limits"][
                        "description_truncate_length"
                    ]
                    suffix = CONSTANTS["text_limits"]["truncate_suffix"]
                    return {
                        "description": line[:truncate_len]
                        + (suffix if len(line) > truncate_len else ""),
                        "docstring": line,
                    }
    except (OSError, UnicodeDecodeError):
...
def summarize_file(filepath: str) -> Dict[str, Any]:
    ...
    if ext == exts["python"]:
        ...
        return {
            "description": CONSTANTS["file_summary"]["skipped_file_prefix"] + filepath
        }
